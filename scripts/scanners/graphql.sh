...
scan_gql() {
    ...
    for dir in "${SCAN_DIRS[@]}"; do
        local matches
        matches=$(grep -rn -E "$pattern" "$dir" --include="*.graphql" --include="*.gql" --include="*.js" --include="*.ts" 2>/dev/null || true)
        if [[ -n "$matches" ]]; then
            results+="$matches"$'\n'
        fi
    done
...
scan_gql_count() {
    ...
    for dir in "${SCAN_DIRS[@]}"; do
        local count
        count=$(grep -rn -E "$pattern" "$dir" --include="*.graphql" --include="*.gql" --include="*.js" --include="*.ts" 2>/dev/null | wc -l || echo "0")
        total=$((total + count))
    done
    echo "$total"
}

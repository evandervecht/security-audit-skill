...
scan_rs() {
    ...
    for dir in "${SCAN_DIRS[@]}"; do
        local matches
        matches=$(grep -rn -E -e "$pattern" "$dir" --include="*.rs" --exclude-dir=target --exclude-dir=.git 2>/dev/null || true)
        if [[ -n "$matches" ]]; then
            results+="$matches"$'\n'
        fi
    done
...
scan_rs_count() {
    ...
    for dir in "${SCAN_DIRS[@]}"; do
        local count
        count=$(grep -rn -E -e "$pattern" "$dir" --include="*.rs" --exclude-dir=target --exclude-dir=.git 2>/dev/null | wc -l || echo "0")
        total=$((total + count))
    done
    echo "$total"
}

...
scan_node() {
    ...
    for dir in "${SCAN_DIRS[@]}"; do
        local matches
        matches=$(grep -rn -E -e "$pattern" "$dir" --include="*.js" --include="*.ts" --include="*.mjs" --include="*.cjs" 2>/dev/null || true)
        if [[ -n "$matches" ]]; then
            results+="$matches"$'\n'
        fi
    done
...
scan_node_count() {
    ...
    for dir in "${SCAN_DIRS[@]}"; do
        local count
        count=$(grep -rn -E -e "$pattern" "$dir" --include="*.js" --include="*.ts" --include="*.mjs" --include="*.cjs" 2>/dev/null | wc -l || echo "0")
        total=$((total + count))
    done
    echo "$total"
}

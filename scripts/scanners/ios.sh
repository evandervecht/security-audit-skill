...
scan_ios() {
    ...
    for dir in "${SCAN_DIRS[@]}"; do
        local matches
        matches=$(grep -rn -E -e "$pattern" "$dir" --include="*.swift" --include="*.m" --include="*.mm" 2>/dev/null || true)
        if [[ -n "$matches" ]]; then
            results+="$matches"$'\n'
        fi
    done
...
scan_pbxproj() {
    local pattern="$1"
    local limit="${2:-5}"
    grep -rn -E -e "$pattern" "$PROJECT_DIR" --include="*.pbxproj" 2>/dev/null | head -"$limit" || true
}

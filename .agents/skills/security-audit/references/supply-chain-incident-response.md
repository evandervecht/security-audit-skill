...
**Note:** Reusable workflows (e.g., `your-org/.github/.github/workflows/reusable.yml@main`) are exempt from SHA pinning requirements. GitHub enforces pinning only on actions (`uses: owner/action@sha`), not on workflow calls (`uses: owner/repo/.github/workflows/file.yml@ref`).
...
pin-github-action --allow "YOUR_ORG/*" .github/workflows/*.yml
...
- 2026-03-20: 59 hardening PRs created across all repos in the org using `pin-github-action`
